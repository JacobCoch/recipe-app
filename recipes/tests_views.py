...
from .models import Recipe, CustomUser as User
from django.core.files.uploadedfile import SimpleUploadedFile

from .views import RecipeDetailView, HomeView, AddRecipe, Profile
...
class HomeViewTestCase(TestCase):
    def setUp(self):
        ...
        self.recipe1 = Recipe.objects.create(
            name="Pasta", ingredients="flour, water, salt, eggs", cooking_time=20, author=self.user
        )
        self.recipe2 = Recipe.objects.create(
            name="Pizza", ingredients="dough, tomato sauce, cheese", cooking_time=30, author=self.user
        )
        # Create more sample recipes for the random suggestions
        for i in range(10):
            Recipe.objects.create(
                name=f"Recipe {i}", ingredients="ingredients", cooking_time=20, author=self.user
            )
    ...
    def test_get_context_data(self):
        ...
        self.assertTrue(all(isinstance(recipe, Recipe) for recipe in random_suggestions))
...
class RecipeListViewTestCase(TestCase):
    def setUp(self):
        ...
        # Create some sample recipes for testing
        Recipe.objects.create(
            name="Pasta", ingredients="flour, water, salt, eggs", cooking_time=20, author_id=self.user.id
        )
        Recipe.objects.create(
            name="Pizza", ingredients="dough, tomato sauce, cheese", cooking_time=30, author_id=self.user.id
        )
...
class RecipeDetailViewTest(TestCase):
    ...
    def test_get_context_data(self):
        recipe = Recipe.objects.create(
            name="Test Recipe", ingredients="ingr, ingr", cooking_time=10, author_id=self.user.id
        )
...
class AddRecipeTestCase(TestCase):
    ...
    def test_form_valid(self):
        ...
        response = self.client.post(self.url, data=form_data)
        self.assertEqual(response.status_code, 302)  # 302 for a successful form submission
...
class DeleteRecipeTestCase(TestCase):
    ...
    def test_delete_recipe_view_get_request(self):
        ...
        self.assertTemplateUsed(response, "recipes/detail.html")  # Use the appropriate template
        self.assertTrue(Recipe.objects.filter(pk=self.recipe.pk).exists())
...
class EditRecipeTestCase(TestCase):
    ...
    def test_edit_recipe_view_with_author(self):
        ...
        self.assertEqual(updated_recipe.ingredients, new_ingredients)  # Check if the ingredients are updated
        self.assertEqual(updated_recipe.cooking_time, new_cooking_time)  # Check if the cooking time is updated
    ...
    def test_edit_recipe_view_with_non_author(self):
        ...
        self.assertEqual(response.status_code, 302)  # Check the redirect status code
        updated_recipe = Recipe.objects.get(pk=self.recipe.pk)
        self.assertNotEqual(updated_recipe.name, new_name)  # Check if the name is not updated
        self.assertNotEqual(updated_recipe.ingredients, new_ingredients)  # Check if the ingredients are not updated
...
class FavedRecipeTestCase(TestCase):
    ...
    def test_faved_recipe_view_add_favorite(self):
        ...
        self.assertTrue(user.fav_recipes.filter(pk=self.recipe.pk).exists())  # Check if the recipe is favorited
    ...
    def test_faved_recipe_view_remove_favorite(self):
        ...
        self.assertEqual(response.status_code, 200)  # Check the response status code
        user = User.objects.get(username="testuser")
        self.assertFalse(user.fav_recipes.filter(pk=self.recipe.pk).exists())  # Check if the recipe is unfavorited


class UpdateProfilePictureTestCase(TestCase):
    def setUp(self):
        self.user = User.objects.create_user(username="testuser", password="testpass")
        self.url = reverse("recipes:update_profile_picture", kwargs={"username": self.user.username})
        self.image = SimpleUploadedFile("profile_pics/profile.jpg", content=b"file_content", content_type="image/jpeg")
