...
class SignUpFormTestCase(TestCase):
    def test_valid_signup_form(self):
        form = SignUpForm(data={"username": "newuser", "password": "newpass123", "confirm_password": "newpass123"})
        self.assertTrue(form.is_valid())

    def test_invalid_signup_form_password_mismatch(self):
      form = SignUpForm(data={"username": "newuser", "password": "newpass123", "confirm_password": "differentpass"})
      self.assertFalse(form.is_valid())
    ...
    def test_invalid_signup_form_missing_username(self):
        form = SignUpForm(data={"password": "newpass123", "confirm_password": "newpass123"})
        self.assertFalse(form.is_valid())
        self.assertIn("username", form.errors)